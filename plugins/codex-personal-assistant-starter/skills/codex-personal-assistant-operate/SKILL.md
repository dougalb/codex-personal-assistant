...
1. Rehydrate from `context/`, active `state/`, attention calibration, and `knowledge/index.md`; retrieve deeper knowledge only when the index points to it and the concept is retrievable under memory-schema v0.3.
...
3. Every worker receives a bounded scope, source set, no durable-write boundary, and completion contract. Require an evidence packet: outcome, claims, source locators, timestamps, confidence, proposed v0.3 memory metadata, freshness/retention proposal, and open questions/conflicts.
...
5. Follow `context/action-policy.md` and `context/data-policy.md`. Read, network access, local edits, and reversible drafts proceed without confirmation only within source and memory policy. Ask once immediately before a final external action or sensitive-memory approval.
